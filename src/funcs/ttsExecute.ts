 ...
 */
export async function ttsExecute(
    client$: AuroraChatbotSDKCore,
    messageId: number,
    options?: RequestOptions
): Promise<
    Result<
        operations.TtsExecuteResponse,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
    ...
    >
> {
    const input$: operations.TtsExecuteRequestBody | undefined = {
        messageId: messageId,
    };

    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.TtsExecuteRequestBody$outboundSchema.optional().parse(value$),
    ...
    const context = { operationID: "tts.execute", oAuth2Scopes: [], securitySource: null };
    ...
    const [result$] = await m$.match<
        ...
        | ConnectionError
    >(
        m$.json(200, operations.TtsExecuteResponse$inboundSchema),
        m$.json(400, operations.TtsExecuteResponse$inboundSchema),
        m$.json(401, operations.TtsExecuteResponse$inboundSchema),
        m$.json(429, operations.TtsExecuteResponse$inboundSchema),
        m$.fail("5XX")
    )(response);
    if (!result$.ok) {
        return result$;
